...
class Trainer():
	...
	def train(self):
		...
		for e in range(self.args.num_epochs):
			...
			eval_train, nodes_embs, logits_train_ind = self.run_epoch(self.splitter.train, e, 'TRAIN', grad = True)
			...
			if len(self.splitter.dev)>0 and e>self.args.eval_after_epochs:  
				eval_valid, _, __ = self.run_epoch(self.splitter.dev, e, 'VALID', grad = False)  
			...
			if len(self.splitter.test)>0 and eval_valid==best_eval_valid and e>self.args.eval_after_epochs:
				eval_test, _, __ = self.run_epoch(self.splitter.test, e, 'TEST', grad = False)
			...
			eval_test_traing, nodes_embs_traing, logits_test_traing = self.run_epoch(self.splitter.test, 10001, 'TEST', grad = False)
			eval_test_ood_traing, nodes_embs_ood_traing, logits_test_ood_traing = self.run_epoch(self.splitter_ood.test, 10002, 'TEST', grad = False)
	...
	def run_epoch(self, split, epoch, set_name, grad):
		...
		return eval_measure, nodes_embs, torch.cat(all_predictions, dim=0) 
