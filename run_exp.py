...
import os
#datasets
from DL import bitcoin_dl as bc
from DL import elliptic_temporal_dl as ell_temp
from DL import uc_irv_mess_dl as ucim
from DL import auto_syst_dl as aus
from DL import sbm_dl as sbm
from DL import reddit_dl as rdt
from DL import brain_dl as brain
import sys
...
def build_dataset(args, ood_mode=None):
	if args.data == 'bitcoinotc' or args.data == 'bitcoinalpha':
		...
		return sbm.sbm_dataset(args, ood_mode)
...
if __name__ == '__main__': 
	...
	# Assign the requested random hyper parameters and store them in args
	...
	# SM for "Structure manipulation" ,FI for "Feature interpolation"
	dataset_ood = build_dataset(args, ood_mode=ood_mode) # None or str: SM FI 
	tasker_ood = build_tasker(args,dataset_ood, ood_mode=ood_mode)  
	...
	trainer.train()
	#detect
	eval_test, nodes_embs, logits_test = trainer.run_epoch(splitter.test, 10001, 'TEST', grad = False)
	# print(f"eval_test, nodes_embs, logits_test_ind:{eval_test}, {nodes_embs.shape}, {logits_test.shape}")
	eval_test_ood, nodes_embs_ood, logits_test_ood = trainer.run_epoch(splitter_ood.test, 10002, 'TEST', grad = False)
	# print(f"eval_test_ood, nodes_embs_ood, logits_test_ood:{eval_test_ood}, {nodes_embs_ood.shape}, {logits_test_ood.shape}")

	with open("output.txt", "a") as f:
		
		sys.stdout = f
		print("******args.data:", args.data)
		print("******args.EDL :", args.EDL)
		if args.EDL in ["evisec","DAEDL"]:
			for method in ["evisec","DAEDL","energy", "entropy", "aleatoric","epistemic","EDL",]:
				detector.evaluate(logits_test, logits_test_ood, method = method, ood_mode = ood_mode)
		else:
